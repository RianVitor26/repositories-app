<template>
  ...
  <el-dialog class="bg-gray-900 absolute z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-5 md:w-3/6 w-11/12"
    ...
    <form @submit.prevent="createRepo" class="flex flex-col">
      ...
      <input v-model="newTitle" minlength="1" maxlength="50" placeholder="Insira o título do repo" required class="mb-3 p-2 bg-gray-800 rounded-md text-gray-100" type="text">
      <label for="description" class="pb-1 font-bold text-gray-100">Descrição</label>
      <textarea v-model="newDesc" minlength="1" maxlength="100" placeholder="Insira a descrição do repo" required name="description" id="description" rows="5" cols="50"
        class="p-2 bg-gray-800 rounded-md text-gray-100"></textarea>
      <select v-model="newLang" required class="my-5 p-2 bg-gray-800 rounded-md text-gray-100" name="languages" id="languages">
